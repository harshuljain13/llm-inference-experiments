#!/usr/bin/env bash
# Mac → Lambda: push module3-engine/ (run from module3-engine/ on your Mac, NOT on the instance).
set -euo pipefail
...
echo "Sync Mac → $LAMBDA:~/module3-engine/"
...
ssh -i "$LAMBDA_SSH_KEY" -o BatchMode=yes "$LAMBDA" \
  'cd ~/module3-engine && rm -f causal_model.py metrics.py gpu_check.py export_notebook.py smol_crew_llm.py tokenizer.py export_notebook.sh'

echo ""
echo "Synced. On Lambda:"
echo "  ssh -i \"\$LAMBDA_SSH_KEY\" \$LAMBDA"
echo "  cd ~/module3-engine && bash scripts/lambda_setup.sh   # first time only"
