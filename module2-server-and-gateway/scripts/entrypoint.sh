...
cd /workspace/module2-server-and-gateway
export PYTHONPATH="/workspace/module2-server-and-gateway:${PYTHONPATH:-}"
...
case "$cmd" in
  ...
  jupyter)
    ...
    exec jupyter lab \
      ...
      --no-browser \
      --allow-root \
      --NotebookApp.token='module2-server-and-gateway' \
...
Class 2 — available commands:
  ...
  jupyter        Open the lab notebook                → http://localhost:8888  (token: module2-server-and-gateway)
